...
public class TicketDAOImpl extends AbstractDao<Integer, TicketEntity> implements TicketDAO {
    ...
    public List<TicketEntity> findAllByEventAndUser(int id_user, String party_name) {
        Query query = getSession().createSQLQuery("SELECT * FROM ticket LEFT JOIN event ON ticket.id_event = event.id_event and event.party_name =:partyName LEFT JOIN user_has_event ON event.id_event = user_has_event.id_event LEFT JOIN user ON event.id_event = user_has_event.id_user WHERE user_has_event.id_user = :idUser")
